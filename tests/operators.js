...
const validSizeOperator = operators.validSizeOperator;
...
const expectShouldParse = (operator, shouldParse) => it(`should have 'shouldParse' set to '${shouldParse}'`, () => {
  expect(operators[operator].parse).to.equal(shouldParse);
});
...
describe('operators', () => {
  ...
  describe(`.${inOp}`, () => {
    it('returns an IN comparator', () => {
      expect(operators[inOp].render(testPath, testValues)).to.equal('path IN (one, two, three)');
    });
    requiresPath(inOp);
    requiresArray(inOp);
    expectShouldParse(inOp, true);
  });
